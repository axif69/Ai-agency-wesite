...
async function dbInsertLead(lead: any): Promise<boolean> {
    ...
    while (true) {
        ...
        for (const lead of newLeads) {
            try {
                ...
                if (website && website !== 'N/A') {
                    ...
                    if (enrichment.email) {
                        db.run("UPDATE leads SET email = ?, status = 'ready', mobile_number = ?, contact_name = ?, linkedin_url = ? WHERE id = ?", [enrichment.email, enrichment.mobile_number, enrichment.contact_name, enrichment.linkedin_url, lead.id]);
        ...
        if (sentToday >= 250) {
            ...
            for (const lead of readyLeads) {
                try {
                    const pitch = await personalizeOutreach(lead.company_name, await scrapeAboutPage(lead.website), tone, model);
                    const result = await sendEmail(lead.email, `${lead.company_name} // Electrical Support`, pitch);
                    if (result.success) {
        ...
        for (const lead of followUps) {
            try {
                ...
                const result = await sendEmail(lead.email, `RE: ${lead.company_name} // Electrical Support`, nudge);
                if (result.success) {
                    db.run("UPDATE leads SET status = 'sent_followup', last_contacted = CURRENT_TIMESTAMP, sent_count = 2 WHERE id = ?", [lead.id]);
