...
// Initial placeholder; will be updated by discoverApiPort()
...
export default function SovereignDashboardV5_1() {
  ...
  useEffect(() => {
    const discoverApiPort = async () => {
      try {
        const res = await fetch('/api_port.json');
        if (res.ok) {
          const data = await res.json();
          const newBase = `http://localhost:${data.port}/api`;
          setApiBase(newBase);
          API_BASE = newBase; // Update legacy reference
          console.log("📡 V2 Dashboard linked to Brain at:", newBase);
        }
      } catch (e) {
        console.log("ℹ️ No discovery file found, using default 3001");
      }
    };
    discoverApiPort();
    
    // Set unique branding
    document.title = "Sovereign .5 | V2 CORE INFRASTRUCTURE";
  }, []);
    ...
    const fetchSettings = async () => {
        try {
            ...
            if (data && Object.keys(data).length > 0) {
                // Ensure boolean strings are converted back to booleans
                const cleaned = { ...data };
  ...
  const handleNinjaScan = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/gmb-ninja-scan`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      <main style={{ flex: 1, padding: '4rem', overflowY: 'auto' }}>
               ...
               <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    ...
                    <GlassCard>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                            <div style={{ fontSize: '0.9rem', display: 'flex', justifyContent: 'space-between' }}><span style={{ opacity: 0.4 }}>Discovery</span> <span style={{ color: ACCENT, fontWeight: 800 }}>YP + BING + GMB ✅</span></div>
                            <div style={{ fontSize: '0.9rem', display: 'flex', justifyContent: 'space-between' }}><span style={{ opacity: 0.4 }}>AI Personalization</span> <span style={{ color: ACCENT, fontWeight: 800 }}>GROQ + MISTRAL ✅</span></div>
                            <div style={{ fontSize: '0.9rem', display: 'flex', justifyContent: 'space-between' }}><span style={{ opacity: 0.4 }}>Email / IMAP Service</span> <span style={{ color: ACCENT, fontWeight: 800 }}>GMAIL ✅</span></div>
                            <div style={{ fontSize: '0.9rem', display: 'flex', justifyContent: 'space-between' }}><span style={{ opacity: 0.4 }}>Relevance Shield</span> <span style={{ color: ACCENT, fontWeight: 800 }}>ACTIVE 🔥</span></div>
        ...
        {view === 'prospects' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
             ...
             {/* v20.0: GMB NINJA STEALTH MINING */}
             <div style={{ marginBottom: 20 }}>
               <div style={{ 
                  ...
                  <div style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center' }}>
                      <div style={{ position: 'absolute', left: 20, color: '#FFD700', opacity: 0.8 }}><Zap size={22} fill="#FFD700" /></div>
                      <input 
                          placeholder="🥷 GMB NINJA: Mine Google Maps (e.g. Cold Room Mfrs, MEP Companies)..." 
                          value={ninjaQuery}
                  ...
                  <button 
                      ...
                      {ninjaLoading ? "MINING GOOGLE..." : "START NINJA MINING"}
                  </button>
               </div>
               <div style={{ fontSize: '0.7rem', opacity: 0.4, marginTop: 10, paddingLeft: 10 }}>
                 🚀 <b>Ninja Tip:</b> This mines Google Maps directly using 'Human-Stealth' browsing. 100% Free Lead Generation.
               </div>
             </div>
                    ...
                    <tbody>
                        ...
                        {sentLeads.map(p => (
                            <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                                ...
                                <td style={{ padding: '1.8rem 1rem', fontSize: '0.8rem' }}>
                                    <div style={{ fontWeight: 600, color: '#fff' }}>{new Date(p.last_contacted || p.added_at).toLocaleDateString()}</div>
                                    <div style={{ fontSize: '0.7rem', opacity: 0.5, marginTop: 4 }}>{new Date(p.last_contacted || p.added_at).toLocaleTimeString()}</div>
                                </td>
                                <td style={{ padding: '1.8rem 1rem' }}>
                        ...
                        <GlassCard>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                <div>
                                    ...
                                    <select 
                                        value={config.model}
                                        onChange={e => { const n = {...config, model: e.target.value}; setConfig(n); saveSettings(n); }}
                                        style={{ width: '100%', padding: '14px', borderRadius: 12, background: '#111', color: '#fff', border: '1px solid rgba(255,255,255,0.1)', outline: 'none' }}>
                                        <option value="llama-3.3-70b-versatile" style={{ background: '#111', color: '#fff' }}>llama-3.3-70b-versatile (Highly Accurate)</option>
                                        <option value="mixtral-8x7b-32768" style={{ background: '#111', color: '#fff' }}>mixtral-8x7b-32768 (Creative & Fast)</option>
                                        <option value="gemma2-9b-it" style={{ background: '#111', color: '#fff' }}>gemma2-9b-it (Efficient)</option>
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <select 
                                        value={config.tone}
                                        onChange={e => { const n = {...config, tone: e.target.value}; setConfig(n); saveSettings(n); }}
                                        style={{ width: '100%', padding: '14px', borderRadius: 12, background: '#111', color: '#fff', border: '1px solid rgba(255,255,255,0.1)', outline: 'none' }}>
                                        <option value="Professional & Bold" style={{ background: '#111', color: '#fff' }}>Professional & Bold</option>
                                        <option value="Concise & Direct" style={{ background: '#111', color: '#fff' }}>Concise & Direct</option>
                                        <option value="Casual & Friendly" style={{ background: '#111', color: '#fff' }}>Casual & Friendly</option>
                                        <option value="Ultra-Premium/Elite" style={{ background: '#111', color: '#fff' }}>Ultra-Premium/Elite</option>
                                    </select>
                                </div>
                            </div>
                        </GlassCard>
                        ...
                        <GlassCard>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                    ...
                                    <div 
                                        onClick={() => { const n = {...config, verify_domains: !config.verify_domains}; setConfig(n); saveSettings(n); }}
                                        style={{ width: 44, height: 24, background: config.verify_domains ? ACCENT : '#333', borderRadius: 20, position: 'relative', cursor: 'pointer', transition: '0.3s' }}>
                                        <div style={{ width: 18, height: 18, background: '#fff', borderRadius: '50%', position: 'absolute', right: config.verify_domains ? 3 : 23, top: 3, transition: '0.3s' }} />
                                    </div>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                    <span style={{ fontSize: '0.9rem', opacity: 0.6 }}>Include Google Business Listings</span>
                                    <div 
                                        onClick={() => { const n = {...config, include_gmb: !config.include_gmb}; setConfig(n); saveSettings(n); }}
