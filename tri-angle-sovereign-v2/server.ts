    ...
    const server = app.listen(port)
        .on('error', (err: any) => {
            if (err.code === 'EADDRINUSE') {
                console.log(`📡 Port ${port} taken, trying ${port + 1}...`);
                startServer(port + 1);
            } else {
        ...
        .on('listening', () => {
            console.log(`\n🚀 SOVEREIGN v13.0 [CORE] ONLINE`);
            console.log(`📡 Backend Brain successfully bound to Port: ${port}`);
            
            // Persist the port for the Dashboard to find
            const portInfo = JSON.stringify({ port });
            try {
                const publicDir = path.join(path.resolve(), 'public');
                if (!fs.existsSync(publicDir)) fs.mkdirSync(publicDir);
                fs.writeFileSync(path.join(publicDir, 'api_port.json'), portInfo);
                console.log(`📄 Port info written to public/api_port.json`);
            } catch (e) {
                console.error('❌ Failed to write port discovery file:', e);
            }
            
            console.log(`👉 DASHBOARD WILL FIND THIS BRAIN AUTOMATICALLY\n`);
        });
}

const DEFAULT_SERVER_PORT = parseInt(process.env.PORT as any) || 3001;
