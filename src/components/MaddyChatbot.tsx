 ...
 * Synchronized with Social Architect: gemini-3-flash-preview
 * Uses direct fetch to bypass SDK resolution issues.
 */
...
2. Maintain an ultra-professional, UAE-corporate tone.
3. Keep replies extremely CONCISE (max 2 sentences).

Conversation Protocol:
- Phase 1: Greet and secure their name.
- Phase 2: Identify objective (Web/App, SEO, Ads, SaaS).
- Phase 3: Gather requirements (Budget, Timeline).
- Phase 4: Secure WhatsApp for Asif Khan.

Interactive Suggestions:
- Always append "[SUGGESTIONS: Option 1, Option 2]" to guide the user.
...
export default function App() {
  ...
  const handleSend = async (overrideInput?: string) => {
    ...
    try {
      // --- ATTEMPT 1: GEMINI 3 (Direct REST) ---
      ...
      console.warn("Primary API Issue, triggering failover...", error);
      ...
      if (GROQ_KEY) {
        try {
          ...
          processResponse(groqData.choices[0].message.content);
        } catch (failoverError) {
          setMessages(prev => [...prev, { role: 'model', text: "Strategic assessment paused. Please message Asif Khan on WhatsApp." }]);
        }
      } else {
