...
/**
 * ELITE GROQ-ONLY CONFIGURATION
 * Using the high-speed Llama 3.3 70B model.
 * Ensure VITE_GROQ_API_KEY is set in your Vercel/Netlify Environment Variables.
 */
const getGroqKey = () => {
  try {
    // Indirect access for build-time safety
...
3. Keep replies extremely CONCISE (max 2 sentences).

Conversation Protocol:
- Phase 1: Greet and secure their name.
- Phase 2: Identify objective (Web/App Dev, SEO/AEO, Ads, SaaS).
- Phase 3: Gather requirements (Budget, Timeline) via single questions.
- Phase 4: Secure WhatsApp/Phone for Asif Khan.

Interactive Suggestions:
- Always append "[SUGGESTIONS: Option 1, Option 2]" at the end of your message.

Tone: Elite, Strategic, and Minimalist.
STRICT RULE: NO MARKDOWN. Never use asterisks (*) or bolding. Use clean, professional text only.
`;
...
export default function App() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { 
      role: 'assistant', 
      text: "Welcome to Asif Digital. I am Khalid, your elite AI Strategic Consultant. To begin our assessment, may I have your name, please?",
   ...
   * Replacing all external TTS APIs with native synthesis for reliability and speed.
   */
  const speak = (text: string) => {
    ...
    // Sanitize text for voice (Strip SUGGESTIONS and Markdown)
  ...
  const processResponse = (rawText: string) => {
    ...
    // Strict Sanitizer: Clear all asterisks before display and voice
  ...
  const handleSend = async (overrideInput?: string) => {
    ...
    try {
      ...
      console.error("Khalid Strategic Error:", error);
      setMessages(prev => [...prev, { 
        role: 'assistant', 
        text: "My strategic assessment lines are momentarily busy. Please contact Asif Khan directly on WhatsApp for an immediate session." 
      }]);
    } finally {
      setIsLoading(false);
    }
  ...
  const forwardToWhatsApp = () => {
    ...
    const text = encodeURIComponent(`*New Strategic Lead Brief*\n\n${history}`);
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                  <div className={`p-4 rounded-2xl text-sm leading-relaxed max-w-[85%] ${
                    msg.role === 'user' 
                      ? 'bg-[#0284c7] text-white rounded-tr-none shadow-lg shadow-sky-900/20' 
              ...
              {leadStage >= 4 && !isLoading && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="pt-2">
                    ...
                    >
                        ...
                        Forward Strategic Brief to Asif
                    </button>
