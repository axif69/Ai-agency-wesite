...
function appName(env) {
  return env && env.APP_NAME ? env.APP_NAME : "Sovereign Sales Engine";
}
...
async function logEvent(env, licenseId, eventType, payload) {
  await env.DB.prepare(
    "INSERT INTO license_events (id, license_id, event_type, payload) VALUES (?, ?, ?, ?)"
  )
    .bind(crypto.randomUUID(), licenseId || null, eventType, JSON.stringify(payload || {}))
    .run();
}

async function findLicenseByKeyHash(env, keyHash) {
...
async function findDeviceByHash(env, deviceHash) {
  return await env.DB.prepare(
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/health") {
      return corsJson({
        ok: true,
        service: "cloudflare-license-server",
        app: appName(env),
      });
    }
    ...
    if (url.pathname === "/api/license/activate" && request.method === "POST") {
      ...
      if (license.status !== "active") return corsJson({ success: false, error: "License is not active." }, 403);
      ...
      if (!existingDevice && activeCount >= Number(license.max_devices || 1)) {
      ...
      const payload = {
        ...
        device_id: deviceId,
        customer_name: customerName || license.customer_name,
      ...
      await logEvent(env, license.id, "activate", payload);
    ...
    if (url.pathname === "/api/license/verify" && request.method === "POST") {
      ...
      const device = await env.DB.prepare(
        ...
        .bind(payload.device_id, payload.license_id)
        .first();
      ...
      await env.DB.prepare("UPDATE devices SET last_check_in = ?, updated_at = ? WHERE id = ?")
        .bind(now, now, payload.device_id)
        .run();
      ...
      await logEvent(env, payload.license_id, "verify", renewedPayload);
    ...
    if (url.pathname === "/api/license/revoke" && request.method === "POST") {
      ...
      if (deviceId) {
        ...
        await logEvent(env, device.license_id, "revoke_device", { device_id: deviceId });
