...
console.log(`📦 [DATABASE] Resale restored to: ${dbPath}`);
...
export const initDB = async (): Promise<void> => {
  return new Promise<void>((resolve, reject) => {
    db.serialize(() => {
      // v23.0: Steel-Door Recovery — Fresh Start Complete
      ...
      // Add a secondary safety purge for legacy construction sectors
      const purgeLegacySql = `DELETE FROM leads WHERE 
        company_name LIKE '%kazema%' OR 
        company_name LIKE '%mep%' OR 
        company_name LIKE '%construction%' OR 
        company_name LIKE '%contracting%' OR
        about_summary LIKE '%portable toilets%'`;
      db.run(purgeLegacySql);
      ...
      const columns = [
        ...
        `ALTER TABLE replies ADD COLUMN body TEXT`,
        `ALTER TABLE replies ADD COLUMN status TEXT DEFAULT 'pending'`,
        `ALTER TABLE replies ADD COLUMN sentiment TEXT DEFAULT 'pending'`,
        `ALTER TABLE leads ADD COLUMN is_relevant INTEGER DEFAULT 1`,
        `ALTER TABLE leads ADD COLUMN analysis_notes TEXT`,
        `ALTER TABLE leads ADD COLUMN target_services TEXT`,
        `ALTER TABLE analytics ADD COLUMN positive_replies INTEGER DEFAULT 0`,
        `ALTER TABLE analytics ADD COLUMN negative_replies INTEGER DEFAULT 0`
      ];
